% ***************************************************************
% *** Matlab code for Uncertainty analysis of synthetic model (Thrust Fault)
...
%% Synthetic model for Thrust fault 
...
grv_obs1=grv_obs;
    ...
    %plot(pc(1,:),pc(2,:),'.')  
     ...
     %title('Relative Misfit in PCA space(Thrust Fault), noise free data')
    ...
    %plot(pc(1,:),pc(2,:),'.')  
     ...
     xlabel('Principal component 1')
     ylabel('Principal component 2')
     %title('Relative Misfit in PCA space(Thrust Fault), noise incorporated data')
     lg1=sprintf('Equivalence function topography (in %%)');
     lg2=sprintf('Best model (relative misfit %2.2f%%)',best_cost);
     lg3=sprintf('True model (relative misfit %2.2f%%)',true_cost);
     legend(lg1,lg2,lg3,'location','best')
