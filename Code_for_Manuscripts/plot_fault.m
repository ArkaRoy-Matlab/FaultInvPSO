...
function grv_cal=plot_fault(x_l,z_l,x_h,z_h,x_l_ext,z_l_ext,x_h_ext,z_h_ext,x_obs,grv_obs,density,fig,clim)

    % [x_l,z_l] and [x_h,z_h] are the vertex points of the inverted Fault plane
    % [x_l_ext,z_l_ext] and [x_h_ext,z_h_ext] are the vertex points of the
    %                                         Fault plane for true model
    % x_obs     = horizontal locations for Observation points
    ...
    %title1 = title for anomaly plot
    %title1 = title for fault structure plot
    %fig = figure number
    ...
    %checking for orientation of the fault
     ...
     %checking the orientation of the fault 
        if dd(1)>dd(2)  
            ...
            p1=plot([x_l_ext x_h_ext x_obs(1) x_obs(1) x_l_ext],[z_l_ext z_h_ext z_h_ext z_l_ext z_l_ext],'r--','linewidth',2) ;
            hold off
            % drawing the legend
            hleg = legend([p1 p2],'location','southeast');
            % legend atributes
            ...
            %plotting the true model using red discontinuous line
            p1=plot([x_l_ext x_obs(end) x_obs(end) x_h_ext x_l_ext],[z_l_ext z_l_ext z_h_ext z_h_ext z_l_ext],'r--','linewidth',2);
            hold off
            % drawing the legend
