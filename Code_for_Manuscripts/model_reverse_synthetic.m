...
%% Synthetic model for Reverse fault  noise free
%upper and lower vertex points of fault plane
x_l_syn=5500; z_l_syn= 300;
x_h_syn=7300; z_h_syn=2800;
...
fprintf('Model for reverse fault having linear depth varying density contrast and for noise free gravity anomalies.\n ')
%Run Model for 10 times and taking best model out of this 10 independent runs
    ...
    save error_energy_Model2_wo.dat bst_err -Ascii  
 %% Results
  %RMSE for gravity  
...
%% Synthetic model for Reverse fault noise incorporated
clear all
%upper and lower vertex points of fault plane
x_l_syn=5500; z_l_syn= 300;
x_h_syn=7300; z_h_syn=2800;
...
%%inversion for synthetic model
fprintf('Model for reverse fault having linear depth varying density contrast and for noisy gravity anomalies.\n ')
%Run Model for 10 times and taking best model out of this 10 independent runs
    for i=1:10
        %running independent model
        [x_l,z_l,x_h,z_h,best_cost,error_energy]=Fault_Invert(grv_obs',x_obs',z_obs,density);
    ...
    grv_cal=plot_fault(x_l,z_l,x_h,z_h,x_l_syn,z_l_syn,x_h_syn,z_h_syn,x_obs,grv_obs,density,fig,clim);
    %saving error energy data
    save error_energy_Model2_w.dat bst_err -Ascii
 %%Results
  %RMSE for gravity  
     N_g=length(grv_obs);  %Number of Observation points 
     %RMSE of given model 
